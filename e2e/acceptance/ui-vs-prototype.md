...
38. **簽核管理**：原型 `/dashboard/admin/signoff`、測試站 `/dashboard/admin/signoff`（`admin-signoff`；票 36）。「各組進度」上方的兩個用途分頁各看一次。**不要按「新增簽核」裡的建立。**
39. **操作紀錄**：原型 `/dashboard/admin/audit`、測試站 `/dashboard/admin/audit`（`admin-audit`；票 36 新頁）。點「管理員」分頁看一次。
