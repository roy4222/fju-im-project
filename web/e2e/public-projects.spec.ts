...
async function publishedItem(cohortId: string, adminId: string, placement: string, title: string, category: string) {
  const itemId = await one(
    `insert into managed_items (id, cohort_id, placement, audience_kind, title, summary, category, created_by_kind, created_by_user_id)
     values (gen_random_uuid(), $1, $2, 'public', $3, '示範摘要', $4, 'user', $5) returning id`,
    [cohortId, placement, title, category, adminId],
  )
...
test.beforeAll(async () => {
  ...
  await pool.query(`update showcase_entries set status = 'published', current_version_id = $2 where id = $1`, [entryId, versionId])
  draftId = await entry(g2, DRAFT)

  await publishedItem(cohortId, adminId, 'honor', HONOR, '校外競賽')
  await publishedItem(cohortId, adminId, 'news', COMPETITION, '競賽資訊')
})
