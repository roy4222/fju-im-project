...
export default async function TeacherHomePage() {
  ...
  const toGrade = (await getGradingQuery().teacherQueue(actor)).filter((e) => e.state !== 'counted').length
  ...
  return (
    <DashboardShell roleLabel="老師" items={TEACHER_NAV} current="/dashboard/teacher">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <Tile label="待評分" value={String(toGrade)} hint="被指派、還沒正式送出的評分" href="/dashboard/teacher/grading" />
      </div>
      <div className="mt-6">
        <EmptyState
          pending
          title="其他老師功能還沒做"
          description="組別繳交與簽核會陸續開放，開放後會出現在這裡。"
        />
      </div>
    </DashboardShell>
  )
