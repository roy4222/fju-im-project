...
/**
 * 學生「簽核」（票 25；原型 `/dashboard/student/signoff`）：自己**此刻所在組別**每個簽核包的目前那一版——
 * 全文、附件、授權範圍、參與者。誰是本人由登入者決定，別組的版本這裡拿不到（版本頁的讀取邊界見 `SignoffQuery.versionDetail`）。
 *
 * 逐人同意／不同意在票 26：這一頁先把要讀的內容與進度位置擺好，按鈕還不出現（不放按了沒反應的假按鈕）。
 * 版本因組員或主指導變更而失效時，上方顯示原因與「等待管理員建立新版」。
 */
export default async function StudentSignoffPage() {
  ...
  return (
    <DashboardShell roleLabel="學生" items={STUDENT_NAV} current="/dashboard/student/signoff">
      <PageHeader title="簽核" description="你的組別要閱讀並同意的內容，每個人只代表自己一票。" />
        ...
        <div className="space-y-6">
          {view.versions.map((v) => (
            <VersionView
              key={v.versionId}
              v={v}
              footnote="「我已閱讀並同意」與「不同意」按鈕在下一階段開放；屆時全部組員各自同意後才輪到指導老師。"
            />
          ))}
        </div>
      )}
