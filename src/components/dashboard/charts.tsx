...
export function Sparkline({ values, className = "", stroke = "var(--primary)" }: { values: number[]; className?: string; stroke?: string }) {
  ...
  return (
    <svg viewBox={`0 0 ${w} ${h}`} className={`h-8 w-24 ${className}`} aria-hidden>
      ...
      <path d={d} fill="none" stroke={stroke} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="chart-line" pathLength={1} />
      <circle cx={pts[pts.length - 1][0]} cy={pts[pts.length - 1][1]} r="2.5" fill={stroke} />
    </svg>
  );
}

export function MiniBars({ values, className = "", fill = "var(--primary)", highlightLast = true }: { values: number[]; className?: string; fill?: string; highlightLast?: boolean }) {
  ...
  return (
    <div className={`flex h-8 items-end gap-[3px] ${className}`} aria-hidden>
      {values.map((v, i) => (
        <span
          key={i}
          className="chart-bar block w-1.5 rounded-sm"
          style={{ height: `${Math.max((v / max) * 100, 8)}%`, background: fill, opacity: highlightLast && i === values.length - 1 ? 1 : 0.35, animationDelay: `${i * 40}ms` }}
        />
      ))}
    </div>
  );
  ...
  return (
    <div className={`relative inline-flex shrink-0 items-center justify-center ${className}`} style={{ width: size, height: size }} role="img" aria-label={`${Math.round(v)}%`}>
      <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} className="-rotate-90">
        ...
        <circle cx={size / 2} cy={size / 2} r={r} fill="none" stroke={color} strokeWidth={stroke} strokeLinecap="round" strokeDasharray={c} strokeDashoffset={c * (1 - v / 100)} className="chart-ring" style={{ ["--ring-c" as string]: c }} />
      </svg>
...
export function SegmentBar({ segments, className = "", height = 8 }: { segments: { value: number; color: string; label: string }[]; className?: string; height?: number }) {
  ...
  return (
    <div className={`flex w-full overflow-hidden rounded-full bg-muted ${className}`} style={{ height }} role="img" aria-label={segments.map((s) => `${s.label} ${s.value}`).join("，")}>
      {segments.map((s) => (
        <span key={s.label} className="chart-seg block h-full" style={{ width: `${(s.value / total) * 100}%`, background: s.color }} />
      ))}
    </div>
  );
}
...
/** 直式長條圖（近 7 天繳交件數等） */
export function BarChart({ data, className = "", color = "var(--primary)", highlight }: { data: { label: string; value: number }[]; className?: string; color?: string; highlight?: number }) {
      ...
      {data.map((d, i) => (
        <div key={d.label} className="group flex flex-1 flex-col items-center gap-1.5">
          ...
          <div className="flex h-24 w-full items-end">
            <span className="chart-bar block w-full rounded-t-md transition-opacity group-hover:opacity-100" style={{ height: `${Math.max((d.value / max) * 100, 4)}%`, background: color, opacity: highlight === i ? 1 : 0.45, animationDelay: `${i * 50}ms` }} />
          </div>
...
/** 橫向比較列（各老師進度、各組進度） */
export function HBar({ label, value, total, color = "var(--primary)", suffix }: { label: string; value: number; total: number; color?: string; suffix?: string }) {
  const pct = total ? Math.round((value / total) * 100) : 0;
  return (
    <div className="flex items-center gap-3">
      <span className="w-20 shrink-0 truncate text-sm">{label}</span>
      <div className="h-2 flex-1 overflow-hidden rounded-full bg-muted">
        <span className="chart-seg block h-full rounded-full" style={{ width: `${pct}%`, background: color }} />
      </div>
