...
/** 管理員：上傳新版同意書 PDF。原型只做前端流程；正式版接檔案儲存並重置所有人的同意。影響範圍用 GROUPS 算。 */
export function ConsentUpload({ current }: { current: { file: string; version: string; updatedAt: string } }) {
  ...
  const teachers = new Set(GROUPS.map((g) => g.advisorId).filter(Boolean)).size;
  return (
    <div className="flex flex-col gap-4 px-5 pb-5">
      <div className="flex flex-wrap items-center gap-3 rounded-lg border border-border bg-muted/30 px-4 py-3 text-sm">
        ...
        <span className="font-semibold">{done ? current.file.split("/").pop()!.replace(current.version, done) : current.file.split("/").pop()}</span>
        <span className="tabular text-xs text-muted-foreground">{done ? "剛剛更新" : `v${current.version}・${current.updatedAt} 上傳`}</span>
      ...
      {done ? (
        <p className="rounded-lg bg-success-subtle px-4 py-3 text-sm font-semibold text-success-on-subtle">已更新同意書 v{done}：{GROUPS.length} 組、{students} 位學生、{teachers} 位老師的同意已重置，登入後會看到新的內容並需重新同意。</p>
      ) : (
        <>
          ...
          {picked ? (
            <div className="flex flex-wrap items-center gap-3 rounded-lg border border-brand/30 bg-brand-subtle px-4 py-3 text-sm">
              <span className="text-brand-on-subtle">上傳新版會把 {GROUPS.length} 組、{students} 位學生與 {teachers} 位老師的同意全部重置，需要重新同意。</span>
