...
2. Google Cloud console：以 Roy 帳號建立專案與 OAuth client（Web application）；consent screen External、Testing 模式並加入 Roy 的測試信箱 A、B（、C）為 test users；Authorized JavaScript origins `https://fju.roy422.dev`；redirect URI `https://fju.roy422.dev/api/auth/callback/google`（本機另加 `http://localhost:3000` 與其 callback）；staging 與正式用同一 client，正式網域時再加第二組。
...
5. GitHub：environment `staging`（required reviewers Roy）的 Secrets `VM_SSH_KEY`、`VM_HOST`、`VM_USER`——只在「第一次 staging 部署」階段（時點待 Roy，預設 S01 後）才建立；推 GHCR 用 `GITHUB_TOKEN`（Actions 權限 Read and write），VM 拉映像用只讀 PAT（`read:packages`）在 VM `docker login`；S00 期間 `cd.yml` 只 dispatch dry-run、不需這些值。Roy 另設 branch protection（`ci.yml` 七道 required checks：`typecheck`、`lint`、`unit`、`integration`、`build`、`e2e-smoke`、`audit`）。
