...
|---|---|---|---|---|---|
| 1 | GitHub repo 保護、Environments、Actions 權限 | 已實際確認：無 branch protection、0 secrets、0 variables、0 environments、0 workflows | 現在先做 | S00 第一個 `web/` PR 合併前 | 契約 05 §2「main 只接受 PR」無法成立 |
...
| 9 | VM 上的三份 `.env` 與資料庫密碼、Better Auth secret | 尚未建立 | 第一次 staging 前 | SOP 02 執行當天 | app、worker、migrate 都起不來 |
...
| 13 | age 備份加密金鑰對與私鑰保管、校方第二把鑰匙 | 尚未建立（本機無 age；文件未記任何公鑰） | 備份整合前 | 同上 | 備份無法加密；還原演練無法解密 |
| 14 | VM 安裝 rclone、age（SOP 01 補項） | 尚未建立 | 備份整合前 | 同上 | 備份服務容器或 cron 無法跑 |
...
| 18 | 平台帳號與金鑰交接校方（Cloudflare、Google Cloud、GitHub、VM、密碼管理器） | 尚未建立 | 正式開放前 | 正式 Gate G6／G7 前 | 校方無法自行維運；備份私鑰只在 Roy 手上 |

## 2. 現在先做

### 2.1 GitHub repo 保護、Environments、Actions 權限
1. **現況**：已實際確認（2026-09-12 `gh`）：`roy4222/fju-im-project` private、default branch `main`、`allow_auto_merge=false`、`delete_branch_on_merge=false`；`branches/main/protection` 回 404；`gh secret list`、`gh variable list`、environments、workflows 全部 0；本機 `gh` 登入 `roy4222`，token scopes `gist, read:org, repo, workflow`。
...
3. **Roy 親自**：（a）Settings → Branches → 新增 `main` 規則：require PR、require status checks（S00 的 `ci.yml` 第一次跑過後才能勾選 check 名稱，先勾 require PR）、禁止 force push；（b）Settings → Environments 建 `staging`（required reviewers：Roy）與 `production`（required reviewers：Roy；等 SOP 06 才用）；（c）Settings → Actions → General：Workflow permissions 設 Read and write（讓 `GITHUB_TOKEN` 能推 GHCR）；（d）勾 `delete_branch_on_merge`（可選）。
...
6. **最晚**：branch protection 在 S00 第一個 `web/` PR 合併前；environments 在 CD 接上前（第一次 staging 部署時點待 Roy）。未完成：契約 05 §2 的「main 只接受 PR」只能標 pending。
7. **驗證與證據**：`gh api repos/roy4222/fju-im-project/branches/main/protection` 回 200 且列出規則；`gh api repos/roy4222/fju-im-project/environments` 列出兩個名稱；輸出（無秘密）貼到 SOP 02 的執行紀錄與 S00 卡「狀態」。
...
2. **帳號**：Cloudflare（Roy 個人）。交接：正式開放前把 zone（或正式網域）與 R2、Turnstile 移到校方 Cloudflare 帳號，或由校方建立自己的帳號後由 Roy 重建資源。
...
5. **精確設定值**：type A、name `fju`、content `140.136.155.167`、proxy **off**（灰雲）、TTL auto；不建 AAAA；staging 與正式共用此名稱（原地轉正，SOP 06），不另建 `staging.` 子網域；分支副本用同 VM 不同 port，不用子網域。
6. **最晚**：SOP 01 執行當天（Caddy 啟動前）。未完成：Caddy 拿不到憑證，站台無 HTTPS，OAuth redirect 驗證失敗。
7. **驗證與證據**：`dig +short fju.roy422.dev A` 回 `140.136.155.167`；`docker compose logs caddy` 出現 certificate obtained；兩段輸出貼入 SOP 01 執行紀錄。
...
5. **精確設定值**：Authorized JavaScript origins `https://fju.roy422.dev`；Authorized redirect URIs `https://fju.roy422.dev/api/auth/callback/google`（Better Auth 預設路徑；staging 與正式同 client，正式網域時再加第二組 origin／URI）；本機開發另加 `http://localhost:3000` 與 `http://localhost:3000/api/auth/callback/google`；環境變數名固定 `GOOGLE_CLIENT_ID`、`GOOGLE_CLIENT_SECRET`（本輪定名，SOP 02 同步）。
...
3. **Roy 親自**：Turnstile → Add widget → hostname `fju.roy422.dev`（正式網域時再加）、widget mode **Managed**；把 site key 與 secret key 填入 VM `.env`。
4. **Fable 可先備**：契約 03 §7 的 siteverify 驗證碼與受控 adapter 測試；local／CI 使用 Cloudflare 官方測試金鑰（永遠通過與永遠失敗兩組），不需 Roy。
5. **精確設定值**：變數名 `TURNSTILE_SITE_KEY`、`TURNSTILE_SECRET_KEY`（本輪定名）；使用位置：密碼註冊、連續登入失敗 5 次後；`action` 值 `register`、`login_retry`。
...
2. **帳號**：GitHub `roy4222`。推映像用 Actions 的 `GITHUB_TOKEN`（需 §2.1 的 Read and write 權限）；VM 拉映像需要一個只讀 PAT（`read:packages`）或 fine-grained token，只存在 VM 的 `docker login` 設定檔。交接：repo 轉校方後重發。
3. **Roy 親自**：建立只讀 PAT（名稱 `fju-vm-ghcr-pull`，scope 僅 `read:packages`，到期 90 天並記到行事曆）；在 VM 以 `deploy` 使用者 `docker login ghcr.io`（token 從剪貼簿貼，不寫檔）。
...
1. **現況**：已實際確認 0 secrets、0 environments。CD 在 S00 只 dry-run，不需要這些值；第一次 staging 部署可以由 Roy 手動 SSH 執行 `deploy.sh`，Secrets 可延到 CD 真正接上。
2. **帳號**：GitHub `roy4222`；SSH 部署金鑰由 Roy 產生（私鑰只放 GitHub Secret，公鑰放 VM `deploy` 使用者）。交接：repo 轉校方後重建。
3. **Roy 親自**：`ssh-keygen -t ed25519 -f fju-deploy -C fju-deploy`（本機）；公鑰加到 VM `deploy` 的 `authorized_keys`；在 environment `staging` 建 Secrets `VM_SSH_KEY`（私鑰內容）、`VM_HOST`、`VM_USER`；用完刪除本機私鑰檔。
4. **Fable 可先備**：`cd.yml` 的 environment 綁定與 `--dry-run` 切換；SOP 03。
5. **精確設定值**：Secrets 名稱固定 `VM_SSH_KEY`、`VM_HOST=140.136.155.167`、`VM_USER=deploy`；`GHCR_TOKEN` 不再需要（改用 `GITHUB_TOKEN`；SOP 02 同步刪除）；environment `staging` required reviewers＝Roy。
...
1. **現況**：尚未建立（本機無 `age`；文件未記錄任何公鑰）。
2. **帳號**：無平台帳號；私鑰在 Roy 密碼管理器＋離線副本。交接：建議備份同時加密給兩個收件者（`age -r <Roy 公鑰> -r <校方公鑰>`），校方公鑰由校方指定人員自行 `age-keygen` 產生並只交出公鑰；這樣交接時不必重加密舊備份。
3. **Roy 親自**：本機 `brew install age`；`age-keygen -o fju-backup.key`；公鑰行貼到 VM `.env.backup` 的 `AGE_PUBLIC_KEY`；私鑰內容存密碼管理器與離線媒體後刪除本機檔；向校方要一把公鑰（可延到正式前）。
4. **Fable 可先備**：SOP 04 的加密／解密命令與還原演練步驟；`.env.backup` 支援多個收件者（`AGE_RECIPIENTS`，逗號分隔；本輪定名，取代單一 `AGE_PUBLIC_KEY` 時 SOP 02 同步）。
...
1. **現況**：尚未建立（SOP 01 目前只裝 docker、compose plugin、ufw；backup 容器內也可自帶，二選一）。
2. **帳號**：VM sudo（Roy）。
...
3. **Roy 親自**：Testing 模式只允許 test users 且 refresh token 7 天到期，正式開放前必須把 Publishing status 改為 In production（只用 `openid/email/profile` 不需 Google 審核）；把校方 Google 帳號加為專案 Owner；正式網域的 origin／redirect URI 加入 client。
...
### 5.3 平台帳號與金鑰交接校方
1. **現況**：尚未建立任何交接文件。
2. **帳號**：Cloudflare（zone、R2、Turnstile）、Google Cloud、GitHub repo 與 GHCR、VM 帳號、密碼管理器條目（A1、三個 DB 密碼、Better Auth secret、age 私鑰、PAT）。
...
1. §2.1 GitHub 保護與 environments（10 分鐘）。
2. §2.3 Cloudflare 登入確認 zone → §3.1 新增 A 記錄（5 分鐘；提早做無害）。
3. §2.4 VM SSH／sudo／對外連線確認（15 分鐘）＋ 向校方送對內 80／443 申請（有前置作業時間，越早越好）。
4. §2.2 準備 Google 測試信箱 → §3.2 Google Cloud 專案、同意畫面、OAuth client、test users（20 分鐘）。
