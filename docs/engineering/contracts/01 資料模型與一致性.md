...
> **2026-09-24 實作回寫**：`OperationLedger` port 新增 `markFailed(tx, recordId)`，把本節「`state='failed'` 只用於已 commit 但後續外部步驟失敗」落成介面；重播（`begin` 回 `replay`）時一併回傳紀錄的 `state`（`committed`／`failed`），用例看到 `failed` 不得回「已完成」的回執。首例是臨時密碼：`setUserPassword` 失敗時，失敗稽核與帳本標 failed 寫在同一交易；重播回 `INTERNAL`，不再回「已核發，無法取回」，也不會偷偷再設一次密碼。（出處 PR #257）
