...
- 母 spec：[issue #6](https://github.com/roy4222/fju-im-project/issues/6)（v3.5 全文含 §9 畫面導覽；頂部表格串起十份子 spec issue 與五份契約）。
...
| 目錄 | 內容 |
|---|---|
| `../ARCHITECTURE.md` | 工程母 spec v3.5（to-spec 結構；§7 落點表回覆 Codex R01–R17、RR01–RR12、A1–A4／B1–B5／O1–O4、C1–C6；§9 畫面導覽；附錄 B＝v2 歷史參考） |
| `contracts/` | 01 v2.3 資料模型與一致性（共用基礎表字典、逐表權限矩陣、`showcase_drafts`、S11 建表順序）、02 v2.1 前後端介面與 UI 整合（Server Action 邊界）、03 v2.1 安全與隱私（`/unlink-account` 封鎖）、04 v2.1 測試與驗收（規則層 A；切片出場 ≠ 年度 PASS）、05 v2.3 CI-CD、部署與維運（變數名、告警通道、插槽 host、token 類型、人工 approval） |
| `modules/` | 01–10 v2.3：模組實作設計（十二節＋附錄 A 資料字典；§7.1 原型畫面對照與 §7.2 缺口；01 撤 session 每人序列化執行器、06 退回鎖序、07 授權範圍從草稿凍結、08 測試事件、09 草稿與版本分開與 S11 建表順序） |
